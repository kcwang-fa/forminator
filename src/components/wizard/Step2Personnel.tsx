...
import { EducationFields } from './personnel/EducationFields';
    ...
    <div>
      ...
      {fields.map((field, index) => (
        <Card
          ...
          extra={
            <Space size={4}>
              ...
              <Button type="text" icon={<ExportOutlined />} size="small" onClick={() => handleExportProfile(getValues(`personnel.${index}`))}>
                匯出 Profile
              </Button>
              {fields.length > 1 && (
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            ...
            <Controller name={`personnel.${index}.name_zh`} control={control} rules={{ required: '請輸入姓名' }}
              render={({ field: f, fieldState }) => (
                <Form.Item label="中文姓名" required validateStatus={fieldState.error ? 'error' : ''}><Input {...f} /></Form.Item>
              )}
            />
            <Controller name={`personnel.${index}.name_en`} control={control}
              render={({ field: f }) => (
                <Form.Item label="英文姓名" tooltip="主持人必填"><Input {...f} /></Form.Item>
              )}
            />
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            <Controller name={`personnel.${index}.gender`} control={control}
              render={({ field: f }) => (
            ...
            />
            <Controller name={`personnel.${index}.birth_date`} control={control}
              render={({ field: f }) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            <Controller name={`personnel.${index}.title`} control={control} rules={{ required: '請輸入職稱' }}
              render={({ field: f, fieldState }) => (
                <Form.Item label="職稱" required validateStatus={fieldState.error ? 'error' : ''}><Input {...f} placeholder="例：醫師" /></Form.Item>
              )}
            />
            <Controller name={`personnel.${index}.unit`} control={control} rules={{ required: '請輸入服務單位' }}
              render={({ field: f, fieldState }) => (
                <Form.Item label="服務單位" required validateStatus={fieldState.error ? 'error' : ''}><Input {...f} /></Form.Item>
              )}
            />
            <Controller name={`personnel.${index}.email`} control={control}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            <Controller name={`personnel.${index}.phone`} control={control} rules={{ required: '請輸入電話' }}
              render={({ field: f, fieldState }) => (
                <Form.Item label="聯絡電話" required validateStatus={fieldState.error ? 'error' : ''}><Input {...f} /></Form.Item>
              )}
            />
            <Controller name={`personnel.${index}.official_phone`} control={control}
              render={({ field: f }) => (
                <Form.Item label="公務電話" tooltip="資料庫申請單使用"><Input {...f} /></Form.Item>
              )}
            />
            <Controller name={`personnel.${index}.id_number`} control={control}
              render={({ field: f }) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <Controller name={`personnel.${index}.fax`} control={control}
              render={({ field: f }) => (<Form.Item label="傳真"><Input {...f} /></Form.Item>)}
            />
            <Controller name={`personnel.${index}.address`} control={control}
              render={({ field: f }) => (
                <Form.Item label="連絡地址" tooltip="署內研究計畫書使用"><Input {...f} /></Form.Item>
              )}
            />
          </div>

          <Controller name={`personnel.${index}.work_description`} control={control}
            render={({ field: f }) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 8, marginTop: 8 }}>
            <Controller name={`personnel.${index}.expertise`} control={control}
              render={({ field: f }) => (
            ...
            />
            <Controller name={`personnel.${index}.irb_training_hours`} control={control}
              render={({ field: f }) => (
                <Form.Item label="研究倫理訓練時數" tooltip="主持人需六年內≥9小時；研究人員需三年內≥4小時" style={{ marginBottom: 8 }}>
                  <InputNumber {...f} min={0} step={1} precision={0} addonAfter="小時" style={{ width: '100%' }} />
                </Form.Item>
              )}
            />
          </div>
      ...
      <Space>
        <Button type="dashed" icon={<PlusOutlined />} onClick={() => append({ ...emptyPersonnel, role: 'researcher' })}>
          新增研究人員
        </Button>
      </Space>
    </div>
