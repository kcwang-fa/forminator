...
// 封面「計畫主持人　簽名：＿＿＿」：圖接在「簽名：」run 後、底線 run 前
// （「簽名：」全文件唯一；底線 run 原樣保留，沒簽名時印出來仍可手簽）
xml = injectSigAfterRun(xml, '簽名：</w:t>', 'pi');

// 附表一末尾「填表人簽章：　　計畫主持人簽章：」（同一段落、在 personnel_appendix loop 內）
//   - 填表人簽章 = 該人自己的簽名（loop 內 member 的 pa_sig）
//   - 計畫主持人簽章 = pi_sig（loop 內查不到的 key docxtemplater 會往外層 scope 找）
// 段尾的 page-break run 不動。
xml = injectSigAfterRun(xml, '填表人簽章：</w:t>', 'pa');
xml = injectSigAfterRun(xml, '計畫主持人簽章：</w:t>', 'pi');

// 健檢：{%pi_sig} 應出現 2 次（封面 + 附表一）、{%pa_sig} 1 次（附表一）
const piSigCount = xml.split('{%pi_sig}').length - 1;
const paSigCount = xml.split('{%pa_sig}').length - 1;
if (piSigCount !== 2 || paSigCount !== 1) {
  throw new Error(`DOC-2 簽名標籤數量錯誤：{%pi_sig}=${piSigCount}（應 2）、{%pa_sig}=${paSigCount}（應 1）`);
}
