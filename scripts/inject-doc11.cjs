// ===== 應用系統維護單（DOC-11）placeholder 注入腳本 =====
...
// 「本欄由權責單位／資訊室填寫」區塊留空。
//
// ⚠️  多數欄位可直接在 label 後 append placeholder，
//     但「需求內容描述」左欄是直排欄位，不能把內容塞進該格，
//     否則文字會被擠成直排；此欄需改寫到右側內容區。
//
...
console.log('📄 Processing DOC-11: 應用系統維護單');
...
// 需求內容描述：
// 左側「需求內容描述」是直排欄位，維持原樣；
// 將右側內容區的提示文字改為 placeholder，避免排版被擠成直排。
xml = xml.replace(
  /<w:t([^>]*)>\(申請人填\)<\/w:t>/,
  '<w:t$1 xml:space="preserve">{doc11_request_desc}</w:t>'
);
